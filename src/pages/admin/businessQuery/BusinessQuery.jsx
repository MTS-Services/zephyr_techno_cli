...
const BusinessQuery = () => {
    ...
    const handleDelete = async (query) => {
        ...
        try {
            ...
            await Swal.fire({
                icon: 'success',
                title: 'Deleted',
                text: payload.message || 'Business query deleted.',
                confirmButtonColor: '#0891b2',
                timer: 2000,
                showConfirmButton: false,
            });
    ...
    return (
        <div onClick={() => setOpenActionMenu(null)}>
            <AdminDashboardTitle
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {queries.map((query, index) => {
                                // If the row is among the last 3, open the dropdown upward to avoid overflow
                                const isLastThree = index >= queries.length - 3;
                                ...
                                return (
                                    <tr key={query.id} className="border-b border-gray-100 hover:bg-gray-50 transition">
                                        ...
                                        <td className="relative px-6 py-4" onClick={(e) => e.stopPropagation()}>
                                            <button
                                                type="button"
                                                onClick={() => setOpenActionMenu(openActionMenu === query.id ? null : query.id)}
                                            ...
                                            {openActionMenu === query.id && (
                                                <div
                                                    className={`absolute right-0 z-20 w-48 rounded-xl border border-gray-200 bg-white p-1 shadow-lg ${
                                                        // Open upward for last 3 rows, downward for the rest
                                                        isLastThree ? 'bottom-10' : 'top-10'
                                                    }`}
                                                >
                                                    <button
                                                        type="button"
